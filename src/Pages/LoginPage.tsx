...
export default function LoginPage() {
  ...
  if (t) {
    if (r !== 'user') {
      setError("Only user accounts can log in here.");
      return;
    }
 ...
 const onSubmit = async (e: React.FormEvent) => {
  ...
  try {
    ...
    // ⚠️ Only allow "user" role here
    if (data.role !== "user") {
      setError("Only normal user accounts can log in from this page.");
      setSubmitting(false);
      return;
    }
  ...
  return (
    <div className=' bg-gradient-to-t from-[#02396c] to-[#001a33] py-10'>


      <div className="flex flex-col md:flex-row w-[90%] bg-[#001a33] rounded-[44px] mx-auto my-10 overflow-hidden">
  ...
  `}
        >
          ...
          <div className="w-[80%] py-8 md:py-0">
            <h2 className="text-3xl md:text-5xl font-sans mb-4 text-[#001a33] text-center md:text-left">Login</h2>
            ...
            <form onSubmit={onSubmit}>
              ...
              <button
                type="submit"
                disabled={submitting}
                className="w-full bg-[#001a33] text-white p-2 rounded disabled:opacity-60 mt-3"
              >
