...
const AdminNavbar: React.FC<UserNavbarProps> = ({ isLoggedInAdmin }) => {
  ...
  return (
     ...
     <nav className=" sticky top-0 z-50 bg-gradient-to-tr from-[#001a33] to-[#02396c]">
      ...
      {/* OVERLAY */}
      <div
        className={`fixed inset-0 bg-black/50 z-40 transition-opacity duration-500 ease-in-out ${isOpen ? "opacity-100 visible" : "opacity-0 invisible"
          }`}
      ...
      {/* MOBILE SIDEBAR */}
      <div
        className={`fixed top-0 left-0 w-64 h-full bg-gradient-to-r  text-gray-200 bg-[#001a33] hover:bg-[#02396c] shadow-lg p-6 flex flex-col space-y-4 z-50 transform transition-transform duration-500 ease-in-out ${isOpen ? "translate-x-0" : "-translate-x-full"
          }`}
      >
